...
2. Compute the new hash. Run `make maturin` first if garaga is not installed in the venv, then run:
   ```python
...
### Step 12b: Update RISC0 verifier class hash

The `risc0_verifier_bn254` contract's class hash is hardcoded in `src/contracts/risc0_sample_app/fibonacci_sequencer/src/lib.cairo` as `RISC_ZERO_VERIFIER_CLASS_HASH`. Run `make update-risc0-class-hash` to recompute and update it automatically.
