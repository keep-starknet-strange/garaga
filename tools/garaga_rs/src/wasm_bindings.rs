...
use num_traits::Num; // Import the Num trait
...
fn jsvalue_to_biguint(v: JsValue) -> Result<BigUint, JsValue> {
    ...
    BigUint::from_str_radix(number_str, radix).map_err(|_| JsValue::from_str("Failed to convert string to BigUint"))
}
...
fn parse_g1_point(value: JsValue) -> Result<G1PointBigUint, JsValue> {
    let obj = value.dyn_into::<js_sys::Object>().map_err(|_| {
        JsValue::from_str("Failed to parse G1PointBigUint")
    })?;
...
fn parse_g2_point(value: JsValue) -> Result<G2PointBigUint, JsValue> {
    let obj = value.dyn_into::<js_sys::Object>().map_err(|_| {
        JsValue::from_str("Failed to parse G2PointBigUint")
    })?;
    let x_tuple = get_property(&obj, "x")?.dyn_into::<Array>().map_err(|_| {
        JsValue::from_str("x is not an array")
    })?;
    let y_tuple = get_property(&obj, "y")?.dyn_into::<Array>().map_err(|_| {
        JsValue::from_str("y is not an array")
    })?;
...
fn parse_biguint_array(value: JsValue) -> Result<Vec<BigUint>, JsValue> {
    let array = value.dyn_into::<Array>().map_err(|_| {
        JsValue::from_str("public_inputs is not an array")
    })?;

    array.iter().map(|v| {

        let s = v.as_string().ok_or_else(|| {
            JsValue::from_str("public_inputs element is not a string")
        })?;
...
fn parse_g1_point_array(value: JsValue) -> Result<Vec<G1PointBigUint>, JsValue> {
    let array = value.dyn_into::<Array>().map_err(|_| JsValue::from_str("ic is not an array"))?;
...
pub fn get_groth16_calldata(
    ...
    let proof_obj = proof_js.dyn_into::<js_sys::Object>().map_err(|_| {
        JsValue::from_str("proof_js is not an object")
    })?;
    ...
    let vk_obj = vk_js.dyn_into::<js_sys::Object>().map_err(|_| {
        JsValue::from_str("proof_js is not an object")
    })?;
...
mod tests {
    ...
    pub fn test_get_groth16_calldata() {
      ...
      assert!(!result_values.is_empty(), "Expected non-empty result values");
    }
    ...
    fn mock_proof_js() -> JsValue {
        let obj = Object::new();
        js_sys::Reflect::set(&obj, &JsValue::from_str("a"), &mock_g1_point("0x2abaae3dd6e4c662f5e071bca525a26d21e2400d01d02c87bce2e8363285990a", "0x24211ff0aa742a6dd1651aadce6f72757beb89de2cff83a6162de7c15674c2")).unwrap();
        js_sys::Reflect::set(&obj, &JsValue::from_str("b"), &mock_g2_point("0x2335f1564c154d7a2ec5d11faf6d991a205bef2858f1687976d0a46502f5224a", "0x223af0bb0912d8ebc535ed489d06cd01fcf4a8ab4596cc28164edf9041d97080", "0x182e8fd86a44983de1d1d9dc4f12f134535b75d39f7aeb21adbf57e1a32ee603", "0xecb11668a0dd5d5031b0837e62ba14222b45718dc101c1278f44a9ed823c16b")).unwrap();
        js_sys::Reflect::set(&obj, &JsValue::from_str("c"), &mock_g1_point("0x290243624a4c11868e7cb0c0f7cfd690dac08e4205d19795b0a8f686dddcdfd6", "0x15de00cc8af159fbdbdc802592e83e1ac61a8026b97e8889b8c5def59ec50b16")).unwrap();

        let public_inputs = Array::new();
        public_inputs.push(&JsValue::from_str("0x1e17db88c1d2e83e49f692cce4bb8169309de90afb2b141156243106aa34b474"));
    ...
    fn mock_vk_js() -> JsValue {
        let obj = Object::new();
        js_sys::Reflect::set(&obj, &JsValue::from_str("alpha"), &mock_g1_point("0xef3cb41414ad20b4fc5891f6b41a263d9f0c0509ee5ab16f866ff6a33fb9e73", "0x1b30646c7d96cf7830a42b1f2f7c0527c38601f1030674b0f14cf7c67de5027b")).unwrap();
        js_sys::Reflect::set(&obj, &JsValue::from_str("beta"), &mock_g2_point("0x21f7077b68f4faa9dcbfdc868e9a0b5c76eb0d62b919772a83b34a8bc1108489", "0x8693a2418c12721dddb0d4f9f3811645c372de7400061b6c6cebc4b3b799bb2", "0x3062e0c71d392fb15f33a8d7f87bf689709cb28f87d554e29dc92d92d1b0ce46", "0x4976fda5a1cd2a69e5b4fd77116cf638be2063a3359e40d8f3e8ec0c90ffd90")).unwrap();
        js_sys::Reflect::set(&obj, &JsValue::from_str("gamma"), &mock_g2_point("0x3541dacedd860ef884359c76cc9c6b81d289d3389e732f5f87195d15a4ce637", "0x2eb7d1fa8416da97272d9cc4aa2e71146c83c13be313e222b376d9a9da02f080", "0x8dce093ad7f7fe20efcb8bbb160b0f2ecb7cd98a7d48b949e86c8b00eef73f9", "0x29a4b793edc639746eb4af5910b954a713c49174d917839a146ea15cd7493f1d")).unwrap();
        js_sys::Reflect::set(&obj, &JsValue::from_str("delta"), &mock_g2_point("0x34fc9ba8eba1e49185580c9ee198f6cac8bc78463d786683ef471af4434acdc", "0x17f115a16975c6f261912be284ecae3a3f0899bb161153d19ebc7f594246b9b0", "0x17fd1ad43bbc8ef931c92abab72ef0ed83a3c195b4dbc7294e1eacf59b3e2054", "0x1bd7d75bf918b6766ff9481052ddb780a4cc43da055b6e98c187b34ca3e7cd1b")).unwrap();

        let ic_array = Array::new();
        ic_array.push(&mock_g1_point("0x124e30bf4dff3ae9c9da665a8bebfd01ce6489a462587fa0f6c879156e7390bc", "0x1fdc3a96cea49b085e101dd2383fdc600dcb4a3450c0471724e1b022791c7532"));
        ic_array.push(&mock_g1_point("0x164668f89d9d6c6126a8465790ba44ddb5716750edaa0ed100691cceb974c6a1", "0x1d9c7309625ec7cf153290e7b5c8497412386c61f3d9d0426a702f368be936da"));
