...
pub fn get_final_exp_witness(
    ...
    if bn254_addchain::pow_exp(&c) != FieldElement::<Degree12ExtensionField>::one() {
        c = &c * &w;
        ws = &ws * &w;
        if bn254_addchain::pow_exp(&c) != FieldElement::<Degree12ExtensionField>::one() {
            c = &c * &w;
    ...
    c = bn254_addchain::pow_r_m_d_inv(&c);
...
fn find_cube_root(
    ...
    let we = bn254_addchain::pow_exp0(&w);
    let mut x = bn254_addchain::pow_exp0(&a);
...
mod tests {
    ...
    fn sample_element() -> FieldElement<Degree12ExtensionField> {
        FieldElement::<Degree12ExtensionField>::new([
            FieldElement::<Degree6ExtensionField>::new([
                FieldElement::<Degree2ExtensionField>::new([
                    FieldElement::<BN254PrimeField>::from_hex_unchecked("2"),
                    FieldElement::<BN254PrimeField>::zero(),
                ]),
                FieldElement::<Degree2ExtensionField>::zero(),
                FieldElement::<Degree2ExtensionField>::zero(),
            ]),
    ...
    fn addchain_matches_pow_custom() {
        ...
        let exp = BigUint::from_str_radix(EXP, 16).unwrap();
        let exp0 = BigUint::from_str_radix(EXP0, 16).unwrap();
        let r_m_d_inv = BigUint::from_str_radix(R_M_D_INV, 16).unwrap();

        assert_eq!(bn254_addchain::pow_exp(&base), pow_custom(&base, &exp));
        assert_eq!(bn254_addchain::pow_exp0(&base), pow_custom(&base, &exp0));
        assert_eq!(
            bn254_addchain::pow_r_m_d_inv(&base),
            pow_custom(&base, &r_m_d_inv)
        );
    }
