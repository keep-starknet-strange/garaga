...
mod tests {
    ...
    fn test_bit_0_case_2() {
        ...
        let xi = xi.into_iter().map(|v| Polynomial::new(v.into_iter().map(|v| FieldElement::<BLS12381PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>())).collect::<Vec<_>>();
        let xj = xj.into_iter().map(|v| v.into_iter().map(|v| FieldElement::<BLS12381PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>()).collect::<Vec<_>>();
        let mut i = vec![];
        let mut j = vec![];
    ...
    fn test_bit_1_init_case_1() {
        ...
        let y = y.into_iter().map(|v| FieldElement::<BLS12381PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>();
        let x = x.into_iter().map(|v| FieldElement::<BLS12381PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>();
        let c = c.into_iter().map(|v| FieldElement::<BLS12381PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>();
        let xf = xf.into_iter().map(|v| FieldElement::<BLS12381PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>();
        let xp = xp.into_iter().map(|(x1, y1, x2, y2)| ([FieldElement::<BLS12381PrimeField>::from_hex(x1).unwrap(), FieldElement::<BLS12381PrimeField>::from_hex(y1).unwrap()], [FieldElement::<BLS12381PrimeField>::from_hex(x2).unwrap(), FieldElement::<BLS12381PrimeField>::from_hex(y2).unwrap()])).collect::<Vec<_>>();
        let xi = xi.into_iter().map(|v| Polynomial::new(v.into_iter().map(|v| FieldElement::<BLS12381PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>())).collect::<Vec<_>>();
        let xj = xj.into_iter().map(|v| v.into_iter().map(|v| FieldElement::<BLS12381PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>()).collect::<Vec<_>>();
    ...
    fn test_bit_1_case_1() {
        let f = ["0x6e2d5c667e253ffe1d604e9b0c0efb376aaa3e1f2fa8194a9fdd627525394de", "0x2dc3c5e55face7c5e2c971b204e63954fbc8ad239d2d152e532c421b7834a99d", "0x2ff47505930441520ea2801041714bcb5d41a0ffa81940aec94ea9901fcac97d", "0xdab9fd3d1ef5226f76a2af1d6b410d8639501aa55867c1f0779e0422d545f0c", "0x2fea029bdb6a10f53ffd60992916c6410e0f53520845bfd3e55f70e8272c8fb3", "0x2f329aa1c588e3e4b7d8f5bedbe3c09728563ff0c46ce45b66636cb6ba1ee436", "0x2290e74d07afff26a405ca3722d951cb9a512f1c100fa50e29907ad3283e7b83", "0x6bd6eba6eb034c98be6fdabbb6f2986a20934fe7f4ccb1aa1cff1983d7d2ae4", "0x1ca0c85d7f94b5c6b3543992e315765cb35d0f7d49149a6939c1e7afec80ee3c", "0x1f70622e79ccbeb35ecce1f7718bba441de2df4f3e916ca84262a68f9eea11f9", "0x2492333e1554b6d2c1ad39913d06e14c6ad3a1569e62952be0e54c3427ea8aab", "0x2a9baf66b1ec17ae1f6f435746d944886953830b40efced523794fd2aa17d4a"];
