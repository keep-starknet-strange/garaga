...
mod tests {
    ...
    fn test_zk_honk_keccak_calldata() -> std::io::Result<()> {
        ...
        let expected_digest = [
            227, 87, 43, 250, 13, 104, 243, 94, 39, 12, 157, 236, 103, 236, 216, 22, 191, 131, 242,
            208, 172, 133, 233, 182, 75, 77, 75, 242, 50, 109, 221, 101,
        ];
    ...
    fn test_zk_honk_starknet_calldata() -> std::io::Result<()> {
        ...
        let digest = Keccak256::digest(&bytes).to_vec();
        let expected_digest = [
            195, 180, 84, 20, 91, 49, 215, 113, 89, 138, 97, 248, 61, 231, 62, 212, 64, 113, 116,
            141, 102, 178, 197, 154, 34, 216, 48, 109, 254, 61, 234, 74,
        ];
        assert_eq!(digest, expected_digest);
        Ok(())
    }
