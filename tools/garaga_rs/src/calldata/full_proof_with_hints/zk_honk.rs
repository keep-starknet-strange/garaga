...
mod tests {
    ...
    fn test_zk_honk_keccak_calldata() -> std::io::Result<()> {
        ...
        let expected_digest = [
            137, 216, 240, 210, 210, 190, 92, 97, 53, 65, 70, 168, 158, 116, 107, 36, 183, 142, 43,
            212, 20, 115, 238, 3, 139, 153, 247, 237, 125, 230, 195, 160,
        ];
    ...
    fn test_zk_honk_starknet_calldata() -> std::io::Result<()> {
        ...
        let digest = Keccak256::digest(&bytes).to_vec();
        let expected_digest = [
            22, 54, 43, 199, 134, 171, 166, 193, 248, 9, 30, 158, 143, 80, 227, 245, 64, 224, 169,
            226, 209, 212, 172, 208, 151, 195, 59, 21, 49, 84, 149, 154,
        ];
        assert_eq!(digest, expected_digest);
        Ok(())
    }
