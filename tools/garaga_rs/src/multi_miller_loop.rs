...
mod tests {
    ...
    fn test_extf_mul_2() {
        ...
        let p = p.into_iter().map(|v| v.into_iter().map(|v| FieldElement::<BLS12381PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>()).collect::<Vec<_>>();
        let xf = xf.into_iter().map(|v| FieldElement::<BLS12381PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>();
        let f = extf_mul(p, 12, None, None, None);
        assert_eq!(f.to_vec(), xf);
    }
    ...
    fn test_precompute_consts_1() {
        ...
        let x = ["0x27783160c70b7ac760cff888c621846a7ed80e50c7ea8a7a718a4ad8a50886fb", "0x10a6d46618419682152a25b5f819b22c2e572d1f670d12026a18c384248dad2f"];
        ...
        let xy = y.into_iter().map(|v| FieldElement::<BN254PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>();
        let xx = x.into_iter().map(|v| FieldElement::<BN254PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>();
    ...
    fn test_precompute_consts_2() {
        ...
        let xy = y.into_iter().map(|v| FieldElement::<BLS12381PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>();
        let xx = x.into_iter().map(|v| FieldElement::<BLS12381PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>();
        let (y, x) = precompute_consts(&p);
        assert_eq!(y.to_vec(), xy);
        assert_eq!(x.to_vec(), xx);
    }
    ...
    #[test]
    fn test_double_1() {
        let q = ("0x1800deef121f1e76426a00665e5c4479674322d4f75edadd46debd5cd992f6ed", "0x198e9393920d483a7260bfb731fb5d25f1aa493335a9e71297e485b7aef312c2", "0x12c85ea5db8c6deb4aab71808dcb408fe3d1e7690c43d37b4ce6cc0166fa7daa", "0x90689d0585ff075ec9e99ad690c3395bc4b313370b38ef355acdadcd122975b");
        let p = ("0x27dc7234fd11d3e8c36c59277c3e6f149d5cd3cfa9a62aee49f8130962b4b3b9", "0x203e205db4f19b37b60121b83a7333706db86431c6d835849957ed8c3928ad79", "0x4bb53b8977e5f92a0bc372742c4830944a59b4fe6b1c0466e2a6dad122b5d2e", "0x195e8aa5b7827463722b8c153931579d3505566b4edf48d498e185f0509de152");
        let l1 = ["0x237db2935c4432bc98005e68cacde68a193b54e64d347301094edcbfa224d3d5", "0x124077e14a7d826a707c3ec1809ae9bafafa05dd6b4ba735fba44e801d415637"];
        let l2 = ["0x3b7178c857630da7676d0000961488f8fbce03349a8dc1dd6e067932b6a7e0d", "0x2b17c2b12c26fdd0e3520b9dfa601ead6f0bf9cd98c81278efe1e96b86397652"];
        let q = (|(x1, y1, x2, y2)| ([FieldElement::<BN254PrimeField>::from_hex(x1).unwrap(), FieldElement::<BN254PrimeField>::from_hex(y1).unwrap()], [FieldElement::<BN254PrimeField>::from_hex(x2).unwrap(), FieldElement::<BN254PrimeField>::from_hex(y2).unwrap()]))(q);
        let xp = (|(x1, y1, x2, y2)| ([FieldElement::<BN254PrimeField>::from_hex(x1).unwrap(), FieldElement::<BN254PrimeField>::from_hex(y1).unwrap()], [FieldElement::<BN254PrimeField>::from_hex(x2).unwrap(), FieldElement::<BN254PrimeField>::from_hex(y2).unwrap()]))(p);
        let xl1 = l1.into_iter().map(|v| FieldElement::<BN254PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>();
        let xl2 = l2.into_iter().map(|v| FieldElement::<BN254PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>();
    ...
    fn test_double_2() {
        ...
        let q = (|(x1, y1, x2, y2)| ([FieldElement::<BLS12381PrimeField>::from_hex(x1).unwrap(), FieldElement::<BLS12381PrimeField>::from_hex(y1).unwrap()], [FieldElement::<BLS12381PrimeField>::from_hex(x2).unwrap(), FieldElement::<BLS12381PrimeField>::from_hex(y2).unwrap()]))(q);
        let xp = (|(x1, y1, x2, y2)| ([FieldElement::<BLS12381PrimeField>::from_hex(x1).unwrap(), FieldElement::<BLS12381PrimeField>::from_hex(y1).unwrap()], [FieldElement::<BLS12381PrimeField>::from_hex(x2).unwrap(), FieldElement::<BLS12381PrimeField>::from_hex(y2).unwrap()]))(p);
        let xl1 = l1.into_iter().map(|v| FieldElement::<BLS12381PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>();
        let xl2 = l2.into_iter().map(|v| FieldElement::<BLS12381PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>();
        let (p, (l1, l2)) = _double(&q);
        assert_eq!(p, xp);
        assert_eq!(l1.to_vec(), xl1);
        assert_eq!(l2.to_vec(), xl2);
    ...
    fn test_double_and_add_1() {
        let qa = ("0x27dc7234fd11d3e8c36c59277c3e6f149d5cd3cfa9a62aee49f8130962b4b3b9", "0x203e205db4f19b37b60121b83a7333706db86431c6d835849957ed8c3928ad79", "0x4bb53b8977e5f92a0bc372742c4830944a59b4fe6b1c0466e2a6dad122b5d2e", "0x195e8aa5b7827463722b8c153931579d3505566b4edf48d498e185f0509de152");
        let qb = ("0x1800deef121f1e76426a00665e5c4479674322d4f75edadd46debd5cd992f6ed", "0x198e9393920d483a7260bfb731fb5d25f1aa493335a9e71297e485b7aef312c2", "0x1d9befcd05a5323e6da4d435f3b617cdb3af83285c2df711ef39c01571827f9d", "0x275dc4a288d1afb3cbb1ac09187524c7db36395df7be3b99e673b13a075a65ec");
        let p = ("0x6064e784db10e9051e52826e192715e8d7e478cb09a5e0012defa0694fbc7f5", "0x1014772f57bb9742735191cd5dcfe4ebbc04156b6878a0a7c9824f32ffb66e85", "0x58e1d5681b5b9e0074b0f9c8d2c68a069b920d74521e79765036d57666c5597", "0x21e2335f3354bb7922ffcc2f38d3323dd9453ac49b55441452aeaca147711b2");
        let l1 = ["0xce69bdf84ed6d6d204fe74db6b371d37e4615ab1b3d578c32d1af5736582972", "0x1e23d69196b41dbf47d406f500e66ea29c8764b3fd262357407c3d96bb3ba710"];
        let l2 = ["0x2cad36e65bbb6f4f41d975b678200fce07c48a5e1ec8ee6f65402483ad127f3a", "0x54c8bc1b50aa258d4fe3a18872139b0287570c3cfa9b8144c3ea2ab524386f5"];
        let l3 = ["0x6567a7f6972b7bbbfa13123614ecf9c4853249bb5ee22ba52a7ed0c533b7173", "0x899cb1e339f7582c42d7517ae6f59453eaf32c7cf422f26ac76a450359f819e"];
        let l4 = ["0x1121d4ca1c2cab3630af75417670de33dfa95eda9f287f4842d688d7afd9cd67", "0x83ef274ba7913a58f14f6c3a2e2c9d74b347bfe7c4c55c27110f2c9a228f7d8"];
        let qa = (|(x1, y1, x2, y2)| ([FieldElement::<BN254PrimeField>::from_hex(x1).unwrap(), FieldElement::<BN254PrimeField>::from_hex(y1).unwrap()], [FieldElement::<BN254PrimeField>::from_hex(x2).unwrap(), FieldElement::<BN254PrimeField>::from_hex(y2).unwrap()]))(qa);
        let qb = (|(x1, y1, x2, y2)| ([FieldElement::<BN254PrimeField>::from_hex(x1).unwrap(), FieldElement::<BN254PrimeField>::from_hex(y1).unwrap()], [FieldElement::<BN254PrimeField>::from_hex(x2).unwrap(), FieldElement::<BN254PrimeField>::from_hex(y2).unwrap()]))(qb);
        let xp = (|(x1, y1, x2, y2)| ([FieldElement::<BN254PrimeField>::from_hex(x1).unwrap(), FieldElement::<BN254PrimeField>::from_hex(y1).unwrap()], [FieldElement::<BN254PrimeField>::from_hex(x2).unwrap(), FieldElement::<BN254PrimeField>::from_hex(y2).unwrap()]))(p);
        let xl1 = l1.into_iter().map(|v| FieldElement::<BN254PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>();
        let xl2 = l2.into_iter().map(|v| FieldElement::<BN254PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>();
        let xl3 = l3.into_iter().map(|v| FieldElement::<BN254PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>();
        let xl4 = l4.into_iter().map(|v| FieldElement::<BN254PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>();
    ...
    fn test_double_and_add_2() {
        ...
        let qa = (|(x1, y1, x2, y2)| ([FieldElement::<BLS12381PrimeField>::from_hex(x1).unwrap(), FieldElement::<BLS12381PrimeField>::from_hex(y1).unwrap()], [FieldElement::<BLS12381PrimeField>::from_hex(x2).unwrap(), FieldElement::<BLS12381PrimeField>::from_hex(y2).unwrap()]))(qa);
        let qb = (|(x1, y1, x2, y2)| ([FieldElement::<BLS12381PrimeField>::from_hex(x1).unwrap(), FieldElement::<BLS12381PrimeField>::from_hex(y1).unwrap()], [FieldElement::<BLS12381PrimeField>::from_hex(x2).unwrap(), FieldElement::<BLS12381PrimeField>::from_hex(y2).unwrap()]))(qb);
        let xp = (|(x1, y1, x2, y2)| ([FieldElement::<BLS12381PrimeField>::from_hex(x1).unwrap(), FieldElement::<BLS12381PrimeField>::from_hex(y1).unwrap()], [FieldElement::<BLS12381PrimeField>::from_hex(x2).unwrap(), FieldElement::<BLS12381PrimeField>::from_hex(y2).unwrap()]))(p);
        let xl1 = l1.into_iter().map(|v| FieldElement::<BLS12381PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>();
        let xl2 = l2.into_iter().map(|v| FieldElement::<BLS12381PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>();
        let xl3 = l3.into_iter().map(|v| FieldElement::<BLS12381PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>();
        let xl4 = l4.into_iter().map(|v| FieldElement::<BLS12381PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>();
        let (p, (l1, l2), (l3, l4)) = _double_and_add(&qa, &qb);
        assert_eq!(p, xp);
        assert_eq!(l1.to_vec(), xl1);
        assert_eq!(l2.to_vec(), xl2);
    ...
    fn test_build_sparse_line_eval_1() {
        let r0 = ["0x237db2935c4432bc98005e68cacde68a193b54e64d347301094edcbfa224d3d5", "0x124077e14a7d826a707c3ec1809ae9bafafa05dd6b4ba735fba44e801d415637"];
        let r1 = ["0x3b7178c857630da7676d0000961488f8fbce03349a8dc1dd6e067932b6a7e0d", "0x2b17c2b12c26fdd0e3520b9dfa601ead6f0bf9cd98c81278efe1e96b86397652"];
        ...
        let xl = ["0x1", "0x3049ed443e0ee79060270c9094ac6390a5c1ab3db65328a47174c6c33f6bd5f2", "0x0", "0xa279a60d390f263fa2b23afda6a2160c7af52ea6fdddb7ffe26ded0e216c821", "0x0", "0x0", "0x0", "0x146bccf3dc47e8eedf574326c5188ea8e4978a568116c9aa5b8517cdeb0e1f38", "0x0", "0x10b71d55b795eb0c1590286e48c412e3434b0def1afddfc8840541b6936825fd", "0x0", "0x0"];
        let r0 = r0.into_iter().map(|v| FieldElement::<BN254PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>();
        let r1 = r1.into_iter().map(|v| FieldElement::<BN254PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>();
        let xl = xl.into_iter().map(|v| FieldElement::<BN254PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>();
    ...
    fn test_build_sparse_line_eval_2() {
        ...
        let r0 = r0.into_iter().map(|v| FieldElement::<BLS12381PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>();
        let r1 = r1.into_iter().map(|v| FieldElement::<BLS12381PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>();
        let xl = xl.into_iter().map(|v| FieldElement::<BLS12381PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>();
    ...
    fn test_bit_0_case_1() {
        ...
        let x = ["0x27783160c70b7ac760cff888c621846a7ed80e50c7ea8a7a718a4ad8a50886fb", "0x10a6d46618419682152a25b5f819b22c2e572d1f670d12026a18c384248dad2f"];
        let xf = ["0x11d12be7502f97a642a86eb73802fe0fe4772283c9e66f3d535534c22390e0ca", "0x109dabf551c2a0d785f86365c9f7a4e2aae4c8e3f5bb96b72c3461402f80049e", "0x15e06e6b765f21da4c21d50705a9bf0f72ba0283f93e75a896235fa107d4ca4d", "0x23c17e1664ac6452ffdd3ca599029d07bbea4e54e00dd8289aa145149627cc90", "0x751d37ad50cc00d6245f08ca81ce16f9a3714c3efe87c95ae8e132234b5dd07", "0x0", "0x1535e3c0e51be5f14093e8501425031896dfb26ec8759e3409e44e94d81e17cf", "0xe07f7cf5446bf6fd7f35bd2c741235928fd0f5cc82d853d0b37dd293fe1a21b", "0x2eae2d9b2332fb1f1c3617826ac6ede574d04ed01692a36b9743fc604b3f3bf", "0x2ab0d3ce9813032acfb54c2e7c43299edb854672b7bc17be60c2e1cda8e826c", "0x1f316d8807400efe60a15b7e5311a9031439eb344c0d84fb80044f8786d59de4", "0x0"];
        let xp = [("0x27dc7234fd11d3e8c36c59277c3e6f149d5cd3cfa9a62aee49f8130962b4b3b9", "0x203e205db4f19b37b60121b83a7333706db86431c6d835849957ed8c3928ad79", "0x4bb53b8977e5f92a0bc372742c4830944a59b4fe6b1c0466e2a6dad122b5d2e", "0x195e8aa5b7827463722b8c153931579d3505566b4edf48d498e185f0509de152"), ("0x19092b3d714b3bb6599b735386dd05eab401e72057bab79773a6de8f5693387b", "0x1acb3fbdde3ce471791558747f0e52b1b7a708834dc533393bcfc2af317703ee", "0x258d7966e4b36382460549a44d5cbe96fbe1c412e36f25218438f72d7d387077", "0x2a58bf6a639ae104439ba4e8db873938a26fcd5bd0b68beb20c44705d9820860")];
        let f = f.into_iter().map(|v| FieldElement::<BN254PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>();
        ...
        let y = y.into_iter().map(|v| FieldElement::<BN254PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>();
        let x = x.into_iter().map(|v| FieldElement::<BN254PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>();
        let xf = xf.into_iter().map(|v| FieldElement::<BN254PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>();
        let xp = xp.into_iter().map(|(x1, y1, x2, y2)| ([FieldElement::<BN254PrimeField>::from_hex(x1).unwrap(), FieldElement::<BN254PrimeField>::from_hex(y1).unwrap()], [FieldElement::<BN254PrimeField>::from_hex(x2).unwrap(), FieldElement::<BN254PrimeField>::from_hex(y2).unwrap()])).collect::<Vec<_>>();
    ...
    fn test_bit_0_case_2() {
        ...
        let f = f.into_iter().map(|v| FieldElement::<BLS12381PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>();
        ...
        let y = y.into_iter().map(|v| FieldElement::<BLS12381PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>();
        let x = x.into_iter().map(|v| FieldElement::<BLS12381PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>();
        let xf = xf.into_iter().map(|v| FieldElement::<BLS12381PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>();
        let xp = xp.into_iter().map(|(x1, y1, x2, y2)| ([FieldElement::<BLS12381PrimeField>::from_hex(x1).unwrap(), FieldElement::<BLS12381PrimeField>::from_hex(y1).unwrap()], [FieldElement::<BLS12381PrimeField>::from_hex(x2).unwrap(), FieldElement::<BLS12381PrimeField>::from_hex(y2).unwrap()])).collect::<Vec<_>>();
    ...
    fn test_bit_1_case_1() {
        ...
        let f = f.into_iter().map(|v| FieldElement::<BN254PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>();
        let q = q.into_iter().map(|(x1, y1, x2, y2)| ([FieldElement::<BN254PrimeField>::from_hex(x1).unwrap(), FieldElement::<BN254PrimeField>::from_hex(y1).unwrap()], [FieldElement::<BN254PrimeField>::from_hex(x2).unwrap(), FieldElement::<BN254PrimeField>::from_hex(y2).unwrap()])).collect::<Vec<_>>();
        let s = s.into_iter().map(|(x1, y1, x2, y2)| ([FieldElement::<BN254PrimeField>::from_hex(x1).unwrap(), FieldElement::<BN254PrimeField>::from_hex(y1).unwrap()], [FieldElement::<BN254PrimeField>::from_hex(x2).unwrap(), FieldElement::<BN254PrimeField>::from_hex(y2).unwrap()])).collect::<Vec<_>>();
        let y = y.into_iter().map(|v| FieldElement::<BN254PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>();
        let x = x.into_iter().map(|v| FieldElement::<BN254PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>();
        let xf = xf.into_iter().map(|v| FieldElement::<BN254PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>();
        let xp = xp.into_iter().map(|(x1, y1, x2, y2)| ([FieldElement::<BN254PrimeField>::from_hex(x1).unwrap(), FieldElement::<BN254PrimeField>::from_hex(y1).unwrap()], [FieldElement::<BN254PrimeField>::from_hex(x2).unwrap(), FieldElement::<BN254PrimeField>::from_hex(y2).unwrap()])).collect::<Vec<_>>();
    ...
    fn test_bit_1_case_2() {
        ...
        let f = f.into_iter().map(|v| FieldElement::<BLS12381PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>();
        let q = q.into_iter().map(|(x1, y1, x2, y2)| ([FieldElement::<BLS12381PrimeField>::from_hex(x1).unwrap(), FieldElement::<BLS12381PrimeField>::from_hex(y1).unwrap()], [FieldElement::<BLS12381PrimeField>::from_hex(x2).unwrap(), FieldElement::<BLS12381PrimeField>::from_hex(y2).unwrap()])).collect::<Vec<_>>();
        let s = s.into_iter().map(|(x1, y1, x2, y2)| ([FieldElement::<BLS12381PrimeField>::from_hex(x1).unwrap(), FieldElement::<BLS12381PrimeField>::from_hex(y1).unwrap()], [FieldElement::<BLS12381PrimeField>::from_hex(x2).unwrap(), FieldElement::<BLS12381PrimeField>::from_hex(y2).unwrap()])).collect::<Vec<_>>();
        let y = y.into_iter().map(|v| FieldElement::<BLS12381PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>();
        let x = x.into_iter().map(|v| FieldElement::<BLS12381PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>();
        let xf = xf.into_iter().map(|v| FieldElement::<BLS12381PrimeField>::from_hex(v).unwrap()).collect::<Vec<_>>();
        let xp = xp.into_iter().map(|(x1, y1, x2, y2)| ([FieldElement::<BLS12381PrimeField>::from_hex(x1).unwrap(), FieldElement::<BLS12381PrimeField>::from_hex(y1).unwrap()], [FieldElement::<BLS12381PrimeField>::from_hex(x2).unwrap(), FieldElement::<BLS12381PrimeField>::from_hex(y2).unwrap()])).collect::<Vec<_>>();
