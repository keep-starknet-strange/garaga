...
use crate::g1point::G1Point;
use crate::utils::{is_quad_residue, sqrt_mod_p, hades_permutation, slope_intercept_internal, line_internal};
...
fn slope_intercept(
    p: G1Point,
    q: G1Point,
) -> PyResult<(FieldElement<Stark252PrimeField>, FieldElement<Stark252PrimeField>)> {
    Ok(slope_intercept_internal(p, q))
}
