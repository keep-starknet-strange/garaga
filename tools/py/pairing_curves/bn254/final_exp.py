...
def mul_torus(y1: list, y2: list, continuable_hash: int):
    num_min_v, continuable_hash = mul_trick_e6(y1, y2, continuable_hash)
    print(f"hash_multorus= {continuable_hash}")
    num_min_v[1] = num_min_v[1] + 1
    num = num_min_v
    den = [y1i + y2i for y1i, y2i in zip(y1, y2)]
    res, continuable_hash = div_trick_e6(num, den, continuable_hash)
    return res, continuable_hash


def div_trick_e6(x: list, y: list, continuable_hash: int) -> (list, int):
    x_gnark, y_gnark = pack_e6(v_to_gnark(x)), pack_e6(v_to_gnark(y))
    div = flatten(div_e6(x_gnark, y_gnark))
    div = gnark_to_v(div)
    check, h = mul_trick_e6(y, div, continuable_hash)
    assert x == check, f"{x} != {check}"
    return div, h


def mul_trick_e6(x: list, y: list, continuable_hash: int) -> (list, int):
    ...
    h = poseidon_hash(r3[0][0] * r3[0][1], h)
    h = poseidon_hash(r3[0][2] * r3[1][0], h)
    h = poseidon_hash(r3[1][1] * r3[1][2], h)
    h = poseidon_hash(r3[2][0] * r3[2][1], h)
    h = poseidon_hash(r3[2][2] * r3[3][0], h)
...
def square_torus(x: list, continuable_hash: int):
    ...
    sq = gnark_to_v(flatten(square_torus_e6(x_gnark)))
    v_tmp = [2 * sq_i - x_i for sq_i, x_i in zip(sq, x)]
    x_poly = to_fp6(v_tmp)
    y_poly = to_fp6(sq)
    z_poly = x_poly * y_poly
    z_polyq = z_poly // unreducible_poly
    ...
    z_polyq_coeffs = z_polyq.get_coeffs()
    z_polyr_coeffs = z_polyr.get_coeffs()
    x3 = [split(e) for e in v_tmp]
    y3 = [split(e) for e in sq]
    q2 = [split_128(e) for e in z_polyq_coeffs]
...
def frobenius_torus(x: list):
    ...
    res = mul_e6((t0, t1, t2), (v0, (0, 0), (0, 0)))
    res = gnark_to_v(flatten(res))
    return res
...
def final_exponentiation(
    ...
    print(f"continuable_hash: {continuable_hash}")
    ...
    t2 = frobenius_torus(t3)
    t0, continuable_hash = mul_torus(t2, t0, continuable_hash)
    t2 = frobenius_square_torus(t4)
    t0, continuable_hash = mul_torus(t2, t0, continuable_hash)
    t2 = inverse_torus(c)
    t2, continuable_hash = mul_torus(t2, t3, continuable_hash)
    t2 = frobenius_cube_torus(t2)

    if unsafe:
        rest, continuable_hash = mul_torus(t2, t0, continuable_hash)
        ...
        if selector1 == 0:
            if is_zero == 0:
                rest, continuable_hash = mul_torus(t2, t0t, continuable_hash)
