...
echo "Building risc0_verifier_bn254..."
(cd "$RISC0_VERIFIER_DIR" && scarb build)
...
# Replace the class hash value in the Cairo file (portable sed -i for macOS and Linux)
if [[ "$OSTYPE" == "darwin"* ]]; then
    sed -i '' "s|^\([[:space:]]*\)0x[0-9a-fA-F]\{1,64\};|\1$CAIRO_CLASS_HASH;|" "$FIBONACCI_SEQUENCER_FILE"
else
    sed -i "s|^\([[:space:]]*\)0x[0-9a-fA-F]\{1,64\};|\1$CAIRO_CLASS_HASH;|" "$FIBONACCI_SEQUENCER_FILE"
fi
