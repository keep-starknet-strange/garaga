...
class MemorySegmentManager:
    ...
    def __init__(self, memory: MemoryDict, prime: int):
        ...
        # A zero segment can be used by any of the builtin runners. Any builtin that needs a zero
        # segment can call add_zero_segment in its initialize_segments method, and call
        # finalize_zero_segment in its finalize_segments method. Since all builtins are initialized
        # together and finalized together in the cairo_runner, they can all use the same zero
        # segment. The first builtin to call add_zero_segment will allocate the segment, and the
        # first builtin to call finalize_zero_segment will finalize it.
