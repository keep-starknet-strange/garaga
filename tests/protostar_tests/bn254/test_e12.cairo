...
from src.bn254.towers.e12 import E12, e12, w_to_gnark_reduced, gnark_to_w, E12full
from src.bn254.towers.e6 import E6, e6, E6full
from src.bn254.towers.e2 import E2, e2
from src.bn254.fq import BigInt3
from src.bn254.pairing import decompress_torus_full
...
func test_add{
    ...
    %{
        ...
        cmd = ['./tools/gnark/main', 'e12', 'add'] + [str(x) for x in inputs]
...
func test_sub{
    ...
    %{
        ...
        cmd = ['./tools/gnark/main', 'e12', 'sub'] + [str(x) for x in inputs]
        out = subprocess.run(cmd, stdout=subprocess.PIPE).stdout.decode('utf-8')
        fp_elements = parse_fp_elements(out)
...
func test_double{
    ...
    %{
        ...
        cmd = ['./tools/gnark/main', 'e12', 'double'] + [str(x) for x in inputs]
        out = subprocess.run(cmd, stdout=subprocess.PIPE).stdout.decode('utf-8')
        fp_elements = parse_fp_elements(out)
...
func test_conjugate{
    ...
    %{
        ...
        cmd = ['./tools/gnark/main', 'e12', 'conjugate'] + [str(x) for x in inputs]
        out = subprocess.run(cmd, stdout=subprocess.PIPE).stdout.decode('utf-8')
        fp_elements = parse_fp_elements(out)
...
func test_decompress_torus{
    ...
    %{
        from tools.py.extension_trick import gnark_to_w, w_to_gnark, gnark_to_v
        inputs=[random.randint(0, P-1) for i in range(24)]

        fill_e6('x', *gnark_to_v(inputs[0:6]))

        cmd = ['./tools/gnark/main', 'e12', 'decompress_torus'] + [str(x) for x in inputs]
        out = subprocess.run(cmd, stdout=subprocess.PIPE).stdout.decode('utf-8')
        fp_elements = parse_fp_elements(out)
        ...
        fill_e12('z', *fp_elements)
    %}
    local x_f: E6full = E6full(x0, x1, x2, x3, x4, x5);

    let res = decompress_torus_full(&x_f);

    let res_gnark = w_to_gnark_reduced([res]);
    e12.assert_E12(res_gnark, z);
    return ();
}
