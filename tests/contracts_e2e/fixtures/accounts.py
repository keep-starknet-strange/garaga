...
from tests.contracts_e2e.utils import (
...
@pytest.fixture(name="account_devnet", scope="package")
def full_node_account(client_devnet: FullNodeClient) -> BaseAccount:
    """
    ...
    return Account(
        address=address,
        client=client_devnet,
        key_pair=KeyPair.from_private_key(int(private_key, 0)),
        chain=StarknetChainId.SEPOLIA,
    )


@pytest.fixture(name="account_sepolia", scope="package")
def full_node_account_sepolia(client_sepolia: FullNodeClient) -> BaseAccount:
    """
    Returns a new Account created with FullNodeClient.
    """
    ...
    return Account(
        address=address,
        client=client_sepolia,
        key_pair=KeyPair.from_private_key(io.to_int(private_key)),
        chain=StarknetChainId.SEPOLIA,
    )
