...
@pytest.mark.parametrize("curve_id", [CurveID.BN254])
@pytest.mark.parametrize("msm_size", [1])
def test_verify_ecip(curve_id, msm_size):
    ...
    scalars = [16 for _ in range(msm_size)]

    zk_ecip_hint(Bs_G1, scalars, use_rust=False)
