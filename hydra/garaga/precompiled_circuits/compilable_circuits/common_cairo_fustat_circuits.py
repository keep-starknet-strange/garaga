...
class EvalFunctionChallengeDuplCircuit(BaseModuloCircuit):
    ...
    @staticmethod
    def _n_coeffs_from_n_points(
        n_points: int, batched: bool
    ...
    @staticmethod
    def _n_points_from_n_coeffs(n_coeffs: int, batched: bool) -> int:
        if batched:
            extra = 4 * 2
        else:
            extra = 0
    ...
    def build_input(self) -> list[PyFelt]:
        ...
        n_coeffs = self._n_coeffs_from_n_points(self.n_points, self.batched)
    ...
    def _run_circuit_inner(self, input: list[PyFelt]) -> ModuloCircuit:
        ...
        n_points = self._n_points_from_n_coeffs(len(all_coeffs), self.batched)
        _log_div_a_num, _log_div_a_den, _log_div_b_num, _log_div_b_den = split_list(
            all_coeffs, self._n_coeffs_from_n_points(n_points, self.batched)
        )
