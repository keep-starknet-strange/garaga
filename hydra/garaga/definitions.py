...
def recode_bits(lst):
    ...
    while i < len(lst):
        if i < len(lst) - 1 and lst[i] == 0 and (lst[i + 1] == 1 or lst[i + 1] == -1):
            if lst[i + 1] == 1:
                result.append(3)  # Replace 01 with 4
            else:
            ...
            result.append(0)  # Replace consecutive zeros with 3
            i += 2
        elif lst[i] == -1:
            result.append(2)  # Replace -1 with 2
            i += 1
...
if __name__ == "__main__":
    recode_bits(jy00(6 * 0x44E992B44A6909F1 + 2)[2:])

    recode_bits([int(x) for x in bin(6 * 0x44E992B44A6909F1 + 2)[2:]][2:])
