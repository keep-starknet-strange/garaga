...
"""
    ...
    with open(os.path.join(tests_dir, f"test_contract.cairo"), "w") as f:
        f.write(
            gen_test_file(
                contract_cairo_name,
...
use {package_name}::{verifier_name}::{{
    I{contract_name}Dispatcher, I{contract_name}DispatcherTrait,
}};
use starknet::ContractAddress;

/// Deploy the {contract_name} contract for testing.
///
/// This function declares and deploys a new instance of the {contract_name} contract
...
/// # Example
/// ```cairo
/// let contract_address = deploy_contract("{contract_name}");
/// let dispatcher = I{contract_name}Dispatcher {{ contract_address }};
/// ```
...
if __name__ == "__main__":
    ...
    for contract_name, vk_path, proof_path in zip(FOLDER_NAMES, vk_paths, proof_paths):
        ...
        with open(
            os.path.join(
                CONTRACTS_FOLDER, contract_name, "tests", f"proof_calldata.txt"
            ),
            "w",
        ) as f:
            for x in cd[1:]:
