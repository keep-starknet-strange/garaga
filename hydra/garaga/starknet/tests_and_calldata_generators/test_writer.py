...
def generate_msm_test(curve_id, n_points, seed):
    ...
    ignored = ignored or (False if n_points in [1, 2, 3] else True)
...
def generate_schnorr_test(curve_id, seed):
    ...
    ignored = False if curve_id in [CurveID.SECP256K1] else True
...
def generate_rsa_test(
    signature: RSA2048Signature, seed: int, suffix: str, should_be_valid: bool
) -> str:
    assertion = "assert!(is_valid);" if should_be_valid else "assert!(!is_valid);"
    return f"""
#[test]
fn test_rsa2048_{suffix}_seed_{seed}() {{
...
def generate_rsa_tests(seed: int) -> str:
    fixtures = [
        (RSA2048Signature.sample(seed=seed), "positive", True),
        (
            RSA2048Signature.tampered_expected_message_sample(seed=seed),
            "tampered_message",
            False,
        ),
        (
            RSA2048Signature.tampered_limb_sample(seed=seed),
            "tampered_limb",
            False,
        ),
    ]
    code = ""
    for signature, suffix, should_be_valid in fixtures:
        code += generate_rsa_test(
            signature=signature,
            seed=seed,
            suffix=suffix,
...
def write_all_tests():
    ...
    with open("src/src/tests/autogenerated/rsa_tests.cairo", "w") as f:
        f.write(generate_rsa_test_file())
