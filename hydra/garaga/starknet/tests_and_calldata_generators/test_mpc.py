...
from garaga.hints.tower_backup import E6, E12, get_tower_object
...
def nondeterministic_extension_field_div(
    A: list[PyFelt], B: list[PyFelt], curve_id: int, extension_degree: int = 6
...
def tower_div(
    curve_id: int, A: list[PyFelt], B: list[PyFelt], extension_degree: int
) -> list[PyFelt]:
...
def extf_inv(
    curve_id: int, Qis, Ris, Y: list[PyFelt], extension_degree: int
) -> list[PyFelt]:
    ...
    Q, R = nondeterministic_extension_field_mul_divmod(
        [y_inv, Y], curve_id, extension_degree
    )
...
def compute_doubling_slope(
    curve_id: int, Q: tuple[list[PyFelt], list[PyFelt]]
) -> list[PyFelt, PyFelt]:
    field = BaseField(CURVES[curve_id].p)
    # num = 3 * x^2
    # den = 2 * y
...
def compute_adding_slope(
    curve_id: int,
    Qa: tuple[list[PyFelt], list[PyFelt]],
    Qb: tuple[list[PyFelt], list[PyFelt]],
) -> list[PyFelt]:
    # num = ya - yb
    # den = xa - xb
...
def build_sparse_line_eval(
    curve_id: int,
    R0: tuple[PyFelt, PyFelt],
    R1: tuple[PyFelt, PyFelt],
    yInv: PyFelt,
    xNegOverY: PyFelt,
) -> list[PyFelt]:
    field = BaseField(CURVES[curve_id].p)
...
def double_step(
    curve_id: int,
    Q: tuple[list[PyFelt], list[PyFelt]],
    k: int,
    yInv: PyFelt,
    xNegOverY: PyFelt,
):
...
def double_and_add_step(
    ...
    (new_x, new_y), (line1R0, line1R1), (line2R0, line2R1) = _double_and_add(
        curve_id, Qa, Qb, k
    )
...
def __bit_0_case(
    curve_id: int,
    f: list[PyFelt],
    points: list[tuple[list[PyFelt], list[PyFelt]]],
    n_pairs: int,
    yInv: list[PyFelt],
    xNegOverY: list[PyFelt],
):
    """
...
def __bit_1_case(
    ...
    f: list[PyFelt],
    points: list[tuple[list[PyFelt], list[PyFelt]]],
    Q_select: list[tuple[list[PyFelt], list[PyFelt]]],
    n_pairs: int,
    yInv: list[PyFelt],
    xNegOverY: list[PyFelt],
):
    ...
    new_points = []
    for k in range(n_pairs):
        T, l1, l2 = double_and_add_step(
            curve_id, points[k], Q_select[k], k, yInv[k], xNegOverY[k]
        )
        new_lines.append(l1)
        new_lines.append(l2)
        new_points.append(T)
...
def _bn254_finalize_step(
    curve_id: int,
    Qs: list[tuple[list[PyFelt], list[PyFelt]]],
    Q: list[tuple[tuple[PyFelt, PyFelt], tuple[PyFelt, PyFelt]]],
):
...
def bn254_finalize_step(
    ...
    for k, (l1, l2) in enumerate(lines):
        line_eval1 = build_sparse_line_eval(
            curve_id, l1[0], l1[1], yInv[k], xNegOverY[k]
        )
        line_eval2 = build_sparse_line_eval(
            curve_id, l2[0], l2[1], yInv[k], xNegOverY[k]
        )
...
def miller_loop(
    ...
    for i in range(start_index - 1, -1, -1):
        if loop_counter[i] == 0:
            ...
            Q_selects = [
                (Q[k] if loop_counter[i] == 1 else Qneg[k]) for k in range(n_pairs)
            ]
...
def multi_pairing_check(
    ...
    c_or_c_inv, scaling_factor, scaling_factor_sparsity = get_root_and_scaling_factor(
        curve_id, P, Q, m
    )
    ...
    if loop_counter[start_index] == 1:
        # Handle case when first bit is +1, need to triple point instead of double and add.
        f, Qs = bit_1_init_case(
            curve_id, f, Q, n_pairs, yInv, xNegOverY, c_inv, Qis, Ris
        )
    elif loop_counter[start_index] == 0:
    ...
    for i in [1, 2, 3] if curve_id == CurveID.BN254.value else [1]:
        _, frobenius_maps[i] = generate_frobenius_maps(
            curve_id=curve_id, extension_degree=12, frob_power=i
        )
    ...
    if curve_id == CurveID.BN254.value:
        ...
        # λ = 6 * x + 2 + q - q**2 + q**3
        c_inv_frob_1 = frobenius(
            curve_id, frobenius_maps, c_inv, 1, extension_degree=12
        )
        c_frob_2 = frobenius(curve_id, frobenius_maps, c, 2, extension_degree=12)
        c_inv_frob_3 = frobenius(
            curve_id, frobenius_maps, c_inv, 3, extension_degree=12
        )
        f = extf_mul(
            curve_id,
            Qis,
...
def multi_pairing_check_result(
    curve_id: CurveID,
    pairs: list[G1G2Pair],
    n_fixed_g2: int,
    ...
    lambda_root, lambda_root_inverse, scaling_factor, Qis, Ris = multi_pairing_check(
        curve_id.value, P, Q, n_fixed_g2, m
    )
...
def hash_hints_and_get_base_random_rlc_coeff(
    curve_id: CurveID,
    pairs: list[G1G2Pair],
    n_fixed_g2: int,
    lambda_root: list[PyFelt],
...
def build_mpcheck_hint(
    ...
    n_fixed_g2: int,
    public_pair: G1G2Pair | None,
) -> tuple[Cairo1SerializableStruct, Cairo1SerializableStruct | None]:
    # Validate input
    n_pairs = len(pairs)
    assert n_pairs >= 2
    ...
    m = (
        extra_miller_loop_result(curve_id, public_pair)
        if public_pair is not None
        else None
    )
    ...
    c0 = hash_hints_and_get_base_random_rlc_coeff(
        curve_id,
        pairs,
        n_fixed_g2,
        lambda_root,
...
def mpc_serialize_to_calldata(
    ...
    lambda_root, lambda_root_inverse, scaling_factor, Ris, big_Q_coeffs, small_Q = (
...
if __name__ == "__main__":
    ...
    def g1g2pair(
        c: CurveID, x1: int, y1: int, x2: int, x3: int, y2: int, y3: int
    ) -> G1G2Pair:
        return G1G2Pair(
            p=G1Point(x=x1, y=y1, curve_id=c),
            q=G2Point(x=(x2, x3), y=(y2, y3), curve_id=c),
            curve_id=c,
        )
