...
def parse_precomputed_g1_consts_and_g2_points(
    ...
    for i in range(n_pairs):
        ...
        current_points.append(split_list_into_pairs(vars[f"Q_{i}"]))
        if bit_1:
            q_or_q_neg_points.append(split_list_into_pairs(vars[f"Q_or_Q_neg_{i}"]))
...
class BaseMPCheckCircuit(BaseEXTFCircuit, ABC):
    def __init__(self, name, curve_id, n_pairs, auto_run=True, compilation_mode=1):
...
class MPCheckBit0Loop(BaseMPCheckCircuit):
    def __init__(
        self,
        curve_id: int,
        n_pairs: int = 0,
...
class MPCheckBit00Loop(BaseMPCheckCircuit):
    def __init__(
        self,
        curve_id: int,
        n_pairs: int = 0,
...
class MPCheckBit1Loop(BaseMPCheckCircuit):
    def __init__(
        self,
        curve_id: int,
        n_pairs: int = 0,
...
class MPCheckPreparePairs(BaseMPCheckCircuit):
    """
...
class MPCheckPrepareLambdaRootEvaluations(BaseMPCheckCircuit):
...
class MPCheckInitBit(BaseMPCheckCircuit):
    def __init__(
        self,
        curve_id: int,
        n_pairs: int,
...
class MPCheckFinalizeBN(BaseMPCheckCircuit):
    def __init__(
        self,
        curve_id: int,
        n_pairs: int,
    ...
    def input_map(self):
        ...
        for i in range(self.n_pairs):
            ...
            input_map[f"yInv_{i}"] = u384
            input_map[f"xNegOverY_{i}"] = u384
            input_map[f"Q{i}"] = G2PointCircuit
        input_map.update(
            {
                "R_n_minus_2": E12D,
                "R_n_minus_1": E12D,
    ...
    def _execute_circuit_logic(
        ...
        current_points = []
        for k in range(n_pairs):
            circuit.Q.append(split_list_into_pairs(vars[f"original_Q{k}"]))
            circuit.yInv.append(vars[f"yInv_{k}"])
            circuit.xNegOverY.append(vars[f"xNegOverY_{k}"])
            current_points.append(split_list_into_pairs(vars[f"Q{k}"]))
...
class MPCheckFinalizeBLS(BaseMPCheckCircuit):
    def __init__(
        self,
        curve_id: int,
        n_pairs: int,
