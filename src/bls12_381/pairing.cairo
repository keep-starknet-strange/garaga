...
func pair_fixed_G2{range_check_ptr}(P: G1Point*) -> E12* {
    alloc_locals;
    let Q = g2.get_g2_generator();
    let f = miller_loop_fixed_Q(P, Q);
    let f = final_exponentiation(f);
    return f;
}
...
func miller_loop_fixed_Q{range_check_ptr}(P: G1Point*, Q: G2Point*) -> E12* {
    ...
    let n = 2;
    with P, Q_original, n {
        let (local final_Q: G2Point*, local result: E12*) = miller_loop_fixed_Q_inner(
            Q=Q, result=result, index=61
        );
    }
...
func miller_loop_fixed_Q_inner{range_check_ptr, P: G1Point*, Q_original: G2Point*, n: felt}(
    Q: G2Point*, result: E12*, index: felt
) -> (point: G2Point*, res: E12*) {
    alloc_locals;
    if (index == -1) {
    ...
    let n = n + 1;
    let (bit: felt) = get_loop_digit(index);
    if (bit == 0) {
        let result = e12.mul_by_014(result, l1.r0, l1.r1);
        %{ print_G2(ids.Q, ids.index, ids.bit) %}
        return miller_loop_inner(Q, result, index - 1);
