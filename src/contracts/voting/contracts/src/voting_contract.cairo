...
trait IVotingContract<TContractState> {
    fn create_voting(ref self: TContractState, question: ByteArray, reveal_date: u64, choices: Array<ByteArray>) -> u64;
...
use core::dict::Felt252Dict;
    use starknet::storage::{
        Map, StoragePointerReadAccess, StoragePointerWriteAccess, StoragePathEntry, Vec, VecTrait, MutableVecTrait
    };
    ...
    struct Storage {
        ...
        voting_to_voters_addresses: Map<u64, Vec<ContractAddress>>, //mapping of a voting id to its voters
        voting_to_voters_bool: Map<u64, Map<ContractAddress, bool>>, //mapping of a voting id to its voters and a bool indicating if they have voted => used to prevent double voting but making it possible to voters change their vote
        voting_to_votes: Map<u64, Map<ContractAddress, Vote>>, //mapping of a voting id to its votes


        voting_to_choice_id_winner: Map<u64, u64>, //mapping of a voting id to the id of the choice that won (only makes sense after the voting is solved)
        voting_to_results: Map<u64, Map<u64, u64>>, //mapping of a voting id to a mapping of a choice id to the number of votes it had (only makes sense after the voting is solved)
    }
    ...
    impl VotingContractImpl of super::IVotingContract<ContractState> {
        fn create_voting(ref self: ContractState, question: ByteArray, reveal_date: u64, choices: Array<ByteArray>) -> u64 {

            let id: u64 = self.votings_count.read() + 1;
            let creator: ContractAddress = get_caller_address();

            self.votings.entry(id).write(
                Voting {
                    id: id,
            ...
            for choice in choices {
                self.voting_to_choices.entry(id).append().write(
                    Choice {
                        label: choice
                    }
                );
        ...
        fn vote(ref self: ContractState, voting_id: u64, choice: u64) {
            ...
            assert(current_time < self.votings.entry(voting_id).read().reveal_date, 'Voting is not open');
            ...
            self.voting_to_votes.entry(voting_id).entry(caller_address).write(
                Vote {
                    choice: choice
                }
            );
        ...
        fn solve_voting(ref self: ContractState, voting_id: u64) -> u64 {
            let current_time: u64 = get_block_timestamp();
            assert(current_time > self.votings.entry(voting_id).read().reveal_date, 'Voting is not closed yet');
            ...
            for i in 0..number_voters {
                let voter: ContractAddress = self.voting_to_voters_addresses.entry(voting_id).at(i).read();

                //TODO: add decryption here
                let choice: u64 = self.voting_to_votes.entry(voting_id).entry(voter).read().choice;
            ...
            for i in 0..number_choices {
                ...
                self.voting_to_results.entry(voting_id).entry(i).write(votes_count.get(i.into()));
            };
            ...
            self.emit(
                VotingSolved {
                    id: voting_id,
                    winner_choice_id: choice_winner
                }
            );
